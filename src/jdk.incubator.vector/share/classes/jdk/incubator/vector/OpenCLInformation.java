...
public class OpenCLInformation implements GPUInformation {
    ...
    private String getMulKernels(){
        return "__kernel void multiplyFIF(__global const float * A, const int factor, __global float * C){int i = get_global_id(0); C[i] = A[i] * factor;} __kernel void multiplyIII(__global const int * A, const int factor, __global int * C){int i = get_global_id(0); C[i] = A[i] * factor;} __kernel void multiplyIFF(__global const int * A, const float factor, __global float * C){int i = get_global_id(0); C[i] = A[i] * factor;} __kernel void multiplyFFF(__global const float * A, const float factor, __global float * C){int i = get_global_id(0); C[i] = A[i] * factor;} __kernel void mulBufferFFF(__global const float * A, __global const float * B, __global float * C){int i = get_global_id(0); C[i] = A[i] * B[i];} __kernel void mulBufferFIF(__global const float * A, __global const int * B, __global float * C){int i = get_global_id(0); C[i] = A[i] * B[i];} __kernel void mulBufferIII(__global const int * A, __global const int * B, __global int * C){int i = get_global_id(0); C[i] = A[i] * B[i];} __kernel void multiplyrange(__global const float * A, __global const float * B, __global float * C, int index1, int index2){int i = get_global_id(0); C[i] = A[i + index1] * B[i + index2];}__kernel void multiplyRepeat(__global float * A, __global float const * B, int size) {int i = get_global_id(0); A[i] = A[i] * B[i % size];}__kernel void multArea(__global const float * A,  __global const float * B, __global float * C, int offset, int widthA, int widthB){int i = get_global_id(0); C[i] = A[i % widthB + offset + widthA * (i / widthB)] * B[i];}";
    }
